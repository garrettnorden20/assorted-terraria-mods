...
namespace BasicMod.Items.Accessories
{
    public class Phaser : ModItem
    {
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            ...
            if ((mp.DashDir == ExampleDashPlayer.DashRight && player.releaseRight == true) ||
                (mp.DashDir == ExampleDashPlayer.DashLeft && player.releaseLeft == true)) {
            ...
            **/
            ...
            if (mp.DashTimer > 0 && mp.DashTimer < ExampleDashPlayer.MAX_DASH_TIMER)
            {
                deactivateTilesBetter(player, 3, 2, 3.0);
            }
            ...
            {
                Dust.NewDust(player.position, player.width, player.height / 2, 211); // blue puffies?
            ...
            if (mp.DashTimer == ExampleDashPlayer.MAX_DASH_TIMER)
            {
                ...
                deactivateTilesBetter(player, 3, 8, 3.0); // this is to clear any previous dash tiles
            ...
            if (mp.DashTimer == 0)
            {
                ...
                mp.MinDashTime = ExampleDashPlayer.MIN_DASH_TIME;
                mp.DashActive = false;
        ...
        public void deactivateTilesBetter(Player p, int deRadius, int additionalBuffer, double firstDistance) // runs each frame
        {
            ...
            for (int i = minTileX; i <= maxTileX; i++)
            {
                for (int j = minTileY; j <= maxTileY; j++)
                {
                    ...
                    //Main.NewText("BOTH LOOPS");
                    if (distanceToTile < (double)firstDistance) // make the tile inactive if distance is small
                    {
                        //Main.NewText("PAST DIFF CHECK");
                        if (Main.tile[i, j] != null && Main.tile[i, j].active())
                        {
        ...
        // this method is very similar to above, but much higher buffer
        public void reactivateTiles(Player p)
        {

            {
                ...
                //Main.NewText("start of deactive");
                int deRadius = 3; // the distance away you want blocks to disappear
                int additionalBuffer = 8; // for checking if out of bounds, to make back to real blocks
                ...
                for (int i = minTileX; i <= maxTileX; i++)
                {
                    for (int j = minTileY; j <= maxTileY; j++)
                    {
                        ...
                        //Main.NewText("BOTH LOOPS");
                        //if (diffX < 1.5 && diffY < 3.0
                        /**
                        if (distanceToTile < -1) // make the tile inactive if distance is small
                        {
                            //Main.NewText("PAST DIFF CHECK");
                            if (Main.tile[i, j] != null && Main.tile[i, j].active())
                            {
        ...
        public class ExampleDashPlayer : ModPlayer
        {
            ...
            public static readonly int MAX_DASH_DELAY = 30;
            public static readonly int MAX_DASH_TIMER = 50;
            ...
            public override void ResetEffects()
            {
                ...
                for (int i = 3; i < 8 + player.extraAccessorySlots; i++)
                {
                    ...
                    if (item.type == ModContent.ItemType<Phaser>())
