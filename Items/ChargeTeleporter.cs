...
namespace BasicMod.Items
{
	...
	class ChargeTeleporter : ModItem
	{
		Point[] chargeLocations;
		int locationIncrementer;
		...
		public override void UseStyle(Player player)
		{
			...
			if (player.itemTime == 0)
			{
				...
				//chargeLocations = BasicWorld.chargePoints;
				if (locationIncrementer >= chargeLocations.Length)
                {
					locationIncrementer = 0;
                }
				Main.NewText("i: " + chargeLocations[locationIncrementer].X);
				Main.NewText("j: " + chargeLocations[locationIncrementer].Y);
				teleportLocation = chargeLocations[locationIncrementer].ToVector2();
			}
