...
namespace BasicMod.Items.MagicWeapons
{
	class LaserPointer : ModItem
	{
		...
		{
			Tooltip.SetDefault("Shoots piercing bullets that home in.");
		}
		...
		public override void SetDefaults()
		{
			...
			item.shoot = mod.ProjectileType("LaserPointerProjectile2");
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			for (int i = 0; i < 1; i++)
			{
				Vector2 perturbedSpeed = new Vector2(speedX, speedY).RotatedBy(MathHelper.ToRadians(degree)); // 30 degree spread.
				Projectile.NewProjectile(position, perturbedSpeed, type, damage, knockBack, Main.myPlayer);

				perturbedSpeed = new Vector2(speedX, speedY).RotatedBy(MathHelper.ToRadians(-1 * degree)); // -30 degree spread.
				Projectile.NewProjectile(position, perturbedSpeed, type, damage, knockBack, Main.myPlayer);
			}
			return true;
		}
	...
	public class LaserPointerProjectile2 : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (endTimer >= END_TIME || !player.channel)
			{
				projectile.Kill();
			}
		...
		private void SetLaserPosition(Player player)
		{
			for (Distance = MOVE_DISTANCE; Distance <= 2200f; Distance += 5f)
			{
				...
				{
					//Point p = start.ToTileCoordinates();
		...
		private void SpawnMeow()
		{
			Vector2 end = projectile.Center + projectile.velocity * Distance;
			end.Y = end.Y - Main.screenHeight;
			//Projectile.NewProjectile(end, Vector2.Zero, 502, 1, 1, projectile.owner);
			Player player = Main.player[projectile.owner];
			...
			for (int i = 0; i < 3; i++)
			{
				Vector2 position = player.Center + new Vector2((-(float)Main.rand.Next(0, 401) * player.direction), -600f);
				position.Y -= (100 * i);
				...
				Projectile.NewProjectile(position.X, position.Y, speedX, speedY, type, 50 * 2, knockBack, player.whoAmI);
